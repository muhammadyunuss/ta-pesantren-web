...
<div class="portlet">
		...
		<div class="portlet-body form">
			<form method="POST" action="{{ route('broadcast-notifikasi.update', $data->id) }}" enctype="multipart/form-data">
			@csrf
			@method("PUT")
            <div class="form-body">
                ...
                <div class="form-group">
                    ...
                    <div>
                        <input type="text" id="judul" name="judul" class="form-control @error('judul') is-invalid @enderror" placeholder="Judul" value="{{ $data->judul }}">
                    </div>
                </div>
                <div class="form-group">
                    <label for="isi">Isi Pemberitahuan</label>
                    <div>
                        <textarea name="isi" id="" cols="30" rows="10">{{ $data->isi }}</textarea>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <select name="walisantri_id" id="walisantri_id" data-with="100%" class="form-control @error('walisantri_id') is-invalid @enderror" required>
                        ...
                        <option value="{{ $s->id }}" {{ old('walisantri_id', $s->id) == $data->walisantri_id  ? 'selected' : null }}>{{ $s->nama_walisantri }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
